...
下面这些在真机上跑通之前，本文件的示例都只是文档契约。第一次实际使用时按顺序做，并把结果写回本文件替换掉假设：

1. `ego-browser --version` 或等效命令，记录实际版本和 helper 名称（`useOrCreateTaskSpace` 一类，还是 `taskSpaces.*` 一类）。
2. 登录状态下打开 `https://www.instagram.com/explore/search/keyword/?q=香港`，记录搜索页是否可读、结果里有多少是个人账号、翻页方式。
3. 打开一个候选主页，读 bio 和前 3 篇帖子，记录一次读取的耗时。
4. 打开该账号的关注列表滚 3 屏，记录加载了多少人、是否出现“稍后再试”或验证。
5. 打开一篇帖子评论区，记录可见评论数、能否展开回复、粤语评论占比。
6. 用这些数字更新主 skill 里“现实预期”一节，并决定是否需要换发现入口。

## 一轮观察示例（未实测）

**以下 helper 名称来自官方 SKILL.md 1.2.6 的文档，与 runtime README 不一致，尚未在任何机器上验证过。** 运行前先用 `ego-browser nodejs` 打一行 `cliLog(Object.keys(globalThis).join(','))` 之类的探测，确认这些名字真的存在；不存在就按已安装版本的官方文档改写，不要修补这份示例让它“看起来能跑”。
