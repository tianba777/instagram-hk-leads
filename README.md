...
| 实际候选、证据、来源与进度写入 SQLite | 已保存 20 个候选并生成固定编号批次；当前 1 个人工通过、5 个排除、14 个待定，自动通过为 0 |
| 真实人工反馈、规则保存、通过名单与种子读取 | 已写入 7 条真实反馈，`learn` 保存 v1 的 7 条保守规则，`next` 读回 1 个人工通过种子，并导出 1 人的通过名单 |
| 合格种子递归及自动模式 | **尚未验证**；尚未实际读取该通过种子的关系，完整范围见 [浏览器实测记录](references/ego-browser.md) |
...
2026-09-05，首批 20 个候选收到 7 条真实反馈：1 个通过、5 个排除、1 个不确定，另有 13 个未审核。反馈已通过 `review` 写入本机数据库，`learn` 已保存规则 v1，`next` 和 Markdown 导出均读回 1 个人工通过账号。当前仍为 `learning`；这只是局部反馈和规则保存的验证，不能据此报告准确率、证明规则有效或提高审核模式。
...
这 37 个测试不访问 Instagram，也不使用真实客户记录；它们验证本地工具行为。浏览器读取和 7 条真实人工反馈另行验证；这些结果仍不能代替完整批次审核、规则效果评估或合格种子递归验证。
