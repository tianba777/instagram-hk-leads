...
4. **扩展**：人工通过（或非 learning 模式下通过固定检查）的账号成为种子。按 `next` 给出的待处理入口读关注、粉丝、多篇帖子的粤语评论；每小段成功后立刻 `progress` 保存，不设固定层数。按可见文字和弹窗标题区分关注／粉丝，即使两者链接都为主页 `#`；滚动弹窗内部，保存本轮实际看到的用户名并去重，不把多次快照数相加。列表跨会话可能重排，历史位置仅作提示，不能按旧索引跳过未见账号。实际帖子可能带用户名前缀，按真实链接登记；明确无评论只结束该帖本轮评论入口，其他帖子未看完则 `posts` 仍为 `in_progress`。私密帖子入口记 `unavailable`，不作人工否决。轮换种子，优先实际带来更多新合格账号的入口。
